...
function atualizarProximo(dados) {
  ...
  let colaboradoresConsiderados = COLABORADORES_ROTACAO.filter(c =>
    true // Removido filtro de colaboradoresPulados
  );
  ...
  if (colaboradoresConsiderados.length === 0) {
    // Se a lista de considerados está vazia, significa que todos já atingiram o limite de saídas diárias.
    // Para manter o ciclo, resetamos a lista para a rotação completa e continuamos a lógica.
    // A lógica subsequente escolherá o que tem a data de saída mais antiga, efetivamente reiniciando o ciclo.
    colaboradoresConsiderados = [...COLABORADORES_ROTACAO];
  }

  // 4. Se não sobrar ninguém, exibe a mensagem e encerra
  if (colaboradoresConsiderados.length === 0) {
  ...
  // 5. Determina a data da última viagem (não-Sede) para cada colaborador considerado
