import { supabase } from './supabaseClient.js';

// --- CONSTANTES GLOBAIS ---
const TODOS_COLABORADORES = ['Maviael', 'Raminho', 'Bruninho', 'Matheus', 'Isaac', 'Mikael'];
const HORAS_DIARIAS_PADRAO = 8;

// --- ELEMENTOS DO DOM ---
const formBancoHoras = document.getElementById('form-banco-horas');
const bancoColaboradorSelect = document.getElementById('banco-colaborador');
const bancoDataInput = document.getElementById('banco-data');
const bancoEntradaInput = document.getElementById('banco-entrada');
const bancoSaidaInput = document.getElementById('banco-saida');
const bancoJustificativaInput = document.getElementById('banco-justificativa');
const btnRegistrarFalta = document.getElementById('btn-registrar-falta');
const btnLancarHorasExtras = document.getElementById('btn-lancar-horas-extras'); // NOVO ELEMENTO
const btnVerResumoHoras = document.getElementById('btn-ver-resumo-horas'); // NOVO ELEMENTO

const buscaColaboradorSelect = document.getElementById('busca-colaborador-select');
const btnBuscarColaborador = document.getElementById('btn-buscar-colaborador');
const resultadoContainer = document.getElementById('resultado-container');
const tituloResultadoBusca = document.getElementById('titulo-resultado-busca');
const saldoGeralContainer = document.getElementById('saldo-geral-container');
const conteudoResultadoBusca = document.getElementById('conteudo-resultado-busca');
const btnPagarHoras = document.getElementById('btn-pagar-horas');
const btnDownloadPdf = document.getElementById('btn-download-pdf');

// Elementos do modal de Ponto (movidos)
const modalPonto = document.getElementById('modal-ponto');
const formPonto = document.getElementById('form-ponto');
const pontoColaboradorSelect = document.getElementById('ponto-colaborador');
const pontoDataInput = document.getElementById('ponto-data');
const pontoTipoSelect = document.getElementById('ponto-tipo');
const pontoObsInput = document.getElementById('ponto-obs');

// Elementos do modal de Lançar Horas Extras (NOVO)
const modalLancarHorasExtras = document.getElementById('modal-lancar-horas-extras');
const formLancarHorasExtras = document.getElementById('form-lancar-horas-extras');
const lancarHorasColaboradorSelect = document.getElementById('lancar-horas-colaborador');
const lancarHorasDataInput = document.getElementById('lancar-horas-data');
const lancarHorasQuantidadeInput = document.getElementById('lancar-horas-quantidade');
const lancarHorasJustificativaInput = document.getElementById('lancar-horas-justificativa');

// Elementos do modal de Resumo de Horas (movido)
const modalResumoHoras = document.getElementById('modal-resumo-horas');
const resumoHorasContentEl = document.getElementById('resumo-horas-content');


// --- DADOS GLOBAIS ---
let colaboradorConsultado = '';

// --- FUNÇÕES AUXILIARES ---

function formatarHorasMinutos(horasDecimais) {
    if (isNaN(horasDecimais)) return 'N/A';
    const horasInteiras = Math.floor(Math.abs(horasDecimais));
    const minutos = Math.round((Math.abs(horasDecimais) - horasInteiras) * 60);
    let sinal = horasDecimais < 0 ? '-' : '';
    if (horasInteiras === 0 && minutos === 0) sinal = ''; // Não mostra sinal para zero

    return `${sinal}${horasInteiras}h ${minutos}m`;
}

function showToast(message) { // Adicionada função showToast, caso ainda não exista
    const toastMessageEl = document.getElementById('toast-message'); // Adicionar ao banco.html se necessário ou usar um global
    if (!toastMessageEl) {
        console.warn('Toast element not found.');
        return;
    }
    toastMessageEl.textContent = message;
    toastMessageEl.classList.add('show');
    setTimeout(() => {
        toastMessageEl.classList.remove('show');
    }, 3000);
}

function abrirModal(modalElement) {
    if (modalElement) modalElement.style.display = 'block';
}

function fecharModal(modalElement) {
    if (modalElement) modalElement.style.display = 'none';
}

// --- FUNÇÕES DE LÓGICA ---

async function registrarHoras(event) {
    event.preventDefault();
    const nome = bancoColaboradorSelect.value;
    const data = bancoDataInput.value;
    const entrada = bancoEntradaInput.value;
    const saida = bancoSaidaInput.value;
    const justificativa = bancoJustificativaInput.value;

    if (!nome || !data || !entrada || !saida) {
        alert('Por favor, preencha todos os campos de data e hora.');
        return;
    }

    const diff = new Date(`${data}T${saida}`) - new Date(`${data}T${entrada}`);
    const horasTrabalhadas = diff / (1000 * 60 * 60);
    const horasExcedentes = horasTrabalhadas - HORAS_DIARIAS_PADRAO;

    const { error } = await supabase.from('banco_horas').insert([{
        colaborador_nome: nome,
        data: data,
        horas_excedentes: horasExcedentes.toFixed(2), // Mantém decimal para cálculo interno
        justificativa: justificativa || `Trabalho do dia ${new Date(data).toLocaleDateString('pt-BR')}`
    }]);

    if (error) {
        alert('Erro ao salvar as horas: ' + error.message);
    } else {
        showToast('Horas registradas com sucesso!');
        formBancoHoras.reset();
        bancoDataInput.valueAsDate = new Date();
        if (colaboradorConsultado === nome) {
            buscarDadosColaborador(nome);
        }
    }
}

async function registrarFalta() {
    const nome = bancoColaboradorSelect.value;
    const data = bancoDataInput.value;
    const justificativa = bancoJustificativaInput.value;

    if (!nome || !data) {
        alert('Selecione o colaborador e a data para registrar a falta.');
        return;
    }

    if (!confirm(`Confirmar registro de FALTA para ${nome} na data ${new Date(data).toLocaleDateString('pt-BR')}?`)) {
        return;
    }

    const { error } = await supabase.from('registros_ponto').insert([{
        colaborador_nome: nome,
        data: data,
        tipo: 'Falta',
        observacao: justificativa || 'Sem observação.'
    }]);

    if (error) {
        alert('Erro ao registrar falta: ' + error.message);
    } else {
        showToast('Falta registrada com sucesso!');
        if (colaboradorConsultado === nome) {
            buscarDadosColaborador(nome);
        }
    }
}

// NOVO: Lançar horas extras manualmente
async function lancarHorasExtras(event) {
    event.preventDefault();
    const nome = lancarHorasColaboradorSelect.value;
    const data = lancarHorasDataInput.value;
    const quantidade = parseFloat(lancarHorasQuantidadeInput.value);
    const justificativa = lancarHorasJustificativaInput.value;

    if (!nome || !data || isNaN(quantidade) || quantidade <= 0 || !justificativa.trim()) {
        alert('Por favor, preencha todos os campos e insira uma quantidade de horas válida.');
        return;
    }

    const { error } = await supabase.from('banco_horas').insert([{
        colaborador_nome: nome,
        data: data,
        horas_excedentes: quantidade.toFixed(2), // Mantém decimal para cálculo interno
        justificativa: justificativa,
        tipo_registro: 'Manual' // Pode ser útil para diferenciar
    }]);

    if (error) {
        alert('Erro ao lançar horas extras: ' + error.message);
    } else {
        showToast('Horas extras lançadas com sucesso!');
        formLancarHorasExtras.reset();
        lancarHorasDataInput.valueAsDate = new Date();
        fecharModal(modalLancarHorasExtras);
        if (colaboradorConsultado === nome) {
            buscarDadosColaborador(nome);
        }
    }
}


// NOVO: Registrar Ponto (anteriormente no script.js)
async function registrarPonto(event) {
    event.preventDefault();
    const nome = pontoColaboradorSelect.value;
    const data = pontoDataInput.value;
    const tipo = pontoTipoSelect.value;
    const observacao = pontoObsInput.value;

    if (!nome || !data || !tipo) {
        alert('Por favor, preencha todos os campos de ponto.');
        return;
    }

    const { error } = await supabase.from('registros_ponto').insert([{
        colaborador_nome: nome,
        data: data,
        tipo: tipo,
        observacao: observacao || 'N/A'
    }]);

    if (error) {
        alert('Erro ao registrar ponto: ' + error.message);
    } else {
        showToast('Ponto registrado com sucesso!');
        formPonto.reset();
        pontoDataInput.valueAsDate = new Date();
        fecharModal(modalPonto);
        // Atualizar lista de ausentes para o 'painel.html' também
        // Isso pode exigir uma chamada para carregarTudo no script.js se o painel estiver aberto.
        // Por enquanto, apenas atualiza se o colaborador estiver sendo consultado aqui.
        if (colaboradorConsultado === nome && tipo.includes('Falta')) {
            buscarDadosColaborador(nome);
        }
    }
}

// NOVO: Resumo de Horas Extras Pendentes (anteriormente no script.js)
async function exibirResumoHorasPendentes() {
    const { data, error } = await supabase.from('banco_horas')
        .select('*')
        .eq('status', 'Pendente')
        .gt('horas_excedentes', 0) // Apenas horas positivas
        .order('colaborador_nome', { ascending: true });

    if (error) {
        resumoHorasContentEl.innerHTML = `<p style="color: red;">Erro ao carregar resumo: ${error.message}</p>`;
        return;
    }

    if (!data || data.length === 0) {
        resumoHorasContentEl.innerHTML = '<p>Nenhuma hora extra pendente encontrada.</p>';
        return;
    }

    const resumoPorColaborador = {};
    data.forEach(item => {
        if (!resumoPorColaborador[item.colaborador_nome]) {
            resumoPorColaborador[item.colaborador_nome] = 0;
        }
        resumoPorColaborador[item.colaborador_nome] += item.horas_excedentes;
    });

    let html = '<ul>';
    for (const nome in resumoPorColaborador) {
        html += `<li><strong>${nome}:</strong> ${formatarHorasMinutos(resumoPorColaborador[nome])}</li>`;
    }
    html += '</ul>';
    resumoHorasContentEl.innerHTML = html;
}

// ATUALIZADO: Adiciona IDs únicos às tabelas geradas
async function buscarDadosColaborador(nome) {
    colaboradorConsultado = nome;
    resultadoContainer.style.display = 'block';
    tituloResultadoBusca.textContent = `Relatório de ${nome}`;
    conteudoResultadoBusca.innerHTML = '<p>Buscando dados...</p>';
    saldoGeralContainer.innerHTML = '';

    const { data: horas, error: errorHoras } = await supabase.from('banco_horas').select('*').eq('colaborador_nome', nome).order('data', { ascending: false });
    const { data: ponto, error: errorPonto } = await supabase.from('registros_ponto').select('*').eq('colaborador_nome', nome).order('data', { ascending: false });
    const { data: historico, error: errorHistorico } = await supabase.from('historico').select('*').ilike('nomes', `%${nome}%`).order('data', { ascending: false });

    if (errorHoras || errorPonto || errorHistorico) {
        conteudoResultadoBusca.innerHTML = `<p style="color: red;">Erro ao consultar dados.</p>`;
        return;
    }

    let saldoTotal = 0;
    if (horas) {
        horas.forEach(h => {
            if (h.status === 'Pendente') {
                saldoTotal += h.horas_excedentes;
            }
        });
    }
    saldoGeralContainer.innerHTML = `Saldo de Horas Pendente: <span class="${saldoTotal >= 0 ? 'saldo-positivo' : 'saldo-negativo'}">${formatarHorasMinutos(saldoTotal)}</span>`;

    let html = '';
    html += '<h4><i class="fa-solid fa-clock"></i> Banco de Horas (Positivo/Negativo)</h4>';
    if (horas && horas.length > 0) {
        html += `<div class="data-table-wrapper"><table class="data-table" id="pdf-table-horas"><thead><tr><th>Data</th><th>Balanço do Dia</th><th>Status</th><th>Justificativa</th></tr></thead><tbody>`;
        horas.forEach(h => {
            html += `<tr><td>${new Date(h.data).toLocaleDateString('pt-BR')}</td><td>${formatarHorasMinutos(h.horas_excedentes)}</td><td>${h.status}</td><td>${h.justificativa}</td></tr>`;
        });
        html += '</tbody></table></div>';
    } else {
        html += '<p>Nenhum registro de horas encontrado.</p>';
    }

    html += '<h4><i class="fa-solid fa-calendar-check"></i> Registros de Faltas</h4>';
    if (ponto && ponto.length > 0) {
        html += `<div class="data-table-wrapper"><table class="data-table" id="pdf-table-faltas"><thead><tr><th>Data</th><th>Ocorrência</th><th>Observação</th></tr></thead><tbody>`;
        ponto.forEach(p => {
            html += `<tr><td>${new Date(p.data).toLocaleDateString('pt-BR')}</td><td>${p.tipo}</td><td>${p.observacao || 'N/A'}</td></tr>`;
        });
        html += '</tbody></table></div>';
    } else {
        html += '<p>Nenhum registro de falta encontrado.</p>';
    }

    html += '<h4><i class="fa-solid fa-truck"></i> Histórico de Viagens</h4>';
    if (historico && historico.length > 0) {
        html += `<div class="data-table-wrapper"><table class="data-table" id="pdf-table-viagens"><thead><tr><th>Data</th><th>Localidade</th><th>Observação</th></tr></thead><tbody>`;
        historico.forEach(h => {
            html += `<tr><td>${new Date(h.data).toLocaleDateString('pt-BR')}</td><td>${h.localidade}</td><td>${h.observacao || 'N/A'}</td></tr>`;
        });
        html += '</tbody></table></div>';
    } else {
        html += '<p>Nenhum registro de viagem encontrado.</p>';
    }
    conteudoResultadoBusca.innerHTML = html;
}

async function pagarHoras() {
    if (!colaboradorConsultado) return;
    if (!confirm(`Você confirma o pagamento de todas as horas EXCEDENTES PENDENTES de ${colaboradorConsultado}?`)) return;

    const { error } = await supabase.from('banco_horas').update({ status: 'Pago' }).eq('colaborador_nome', colaboradorConsultado).eq('status', 'Pendente').gt('horas_excedentes', 0);

    if (error) {
        alert(`Erro ao registrar pagamento: ${error.message}`);
    } else {
        showToast(`Pagamento registrado com sucesso!`);
        buscarDadosColaborador(colaboradorConsultado);
    }
}

// ATUALIZADO: Lógica de geração de PDF corrigida e mais robusta
async function gerarPDF() {
    if (!colaboradorConsultado) {
        alert("Primeiro, consulte um colaborador para gerar o PDF.");
        return;
    }

    const { jsPDF } = window.jspdf;
    const doc = new jsPDF();
    const nome = colaboradorConsultado;
    const saldoHorasText = saldoGeralContainer.innerText;

    // Seleciona as tabelas por seus IDs únicos
    const tableHoras = document.getElementById('pdf-table-horas');
    const tableFaltas = document.getElementById('pdf-table-faltas');
    const tableViagens = document.getElementById('pdf-table-viagens');

    doc.setFontSize(18);
    doc.text(`Relatório do Colaborador: ${nome}`, 14, 22);
    doc.setFontSize(11);
    doc.setTextColor(100);

    let finalY = 30;

    // Helper para adicionar tabelas ao PDF somente se elas existirem no HTML
    const addTableToPdf = (title, tableElement, yPos) => {
        doc.setFontSize(14);
        doc.text(title, 14, yPos);
        yPos += 7;
        if (tableElement) {
            doc.autoTable({
                html: tableElement,
                startY: yPos,
                theme: 'striped',
                headStyles: { fillColor: [58, 80, 107] }
            });
            return doc.autoTable.previous.finalY + 10;
        } else {
            doc.setFontSize(11);
            doc.text("Nenhum registro encontrado para esta categoria.", 14, yPos);
            return yPos + 10;
        }
    };

    finalY = addTableToPdf("Banco de Horas", tableHoras, finalY);
    doc.setFontSize(12);
    doc.text(saldoHorasText, 14, finalY);
    finalY += 10;

    finalY = addTableToPdf("Registros de Faltas", tableFaltas, finalY);
    finalY = addTableToPdf("Histórico de Viagens", tableViagens, finalY);

    doc.save(`relatorio_${nome.toLowerCase().replace(' ', '_')}_${new Date().toISOString().slice(0,10)}.pdf`);
}

// --- INICIALIZAÇÃO E EVENT LISTENERS ---
document.addEventListener('DOMContentLoaded', () => {
    const optionsHtml = TODOS_COLABORADORES.map(nome => `<option value="${nome}">${nome}</option>`).join('');
    bancoColaboradorSelect.innerHTML = optionsHtml;
    buscaColaboradorSelect.innerHTML = '<option value="" disabled selected>Selecione um nome</option>' + optionsHtml;
    pontoColaboradorSelect.innerHTML = optionsHtml; // Populando select do modal de ponto
    lancarHorasColaboradorSelect.innerHTML = optionsHtml; // Populando select do modal de horas extras
    
    bancoDataInput.valueAsDate = new Date();
    pontoDataInput.valueAsDate = new Date(); // Definindo data atual para modal de ponto
    lancarHorasDataInput.valueAsDate = new Date(); // Definindo data atual para modal de horas extras

    formBancoHoras.addEventListener('submit', registrarHoras);
    btnRegistrarFalta.addEventListener('click', registrarFalta);
    
    // Event listeners para o novo botão de lançar horas extras e ver resumo
    btnLancarHorasExtras.addEventListener('click', () => abrirModal(modalLancarHorasExtras));
    btnVerResumoHoras.addEventListener('click', async () => {
        await exibirResumoHorasPendentes();
        abrirModal(modalResumoHoras);
    });

    // Event listeners para os formulários dos modais
    formPonto.addEventListener('submit', registrarPonto);
    formLancarHorasExtras.addEventListener('submit', lancarHorasExtras);

    // Event listeners para fechar modais ao clicar no X (data-modal)
    document.querySelectorAll('.modal').forEach(modal => {
        modal.addEventListener('click', e => {
            if (e.target.classList.contains('modal-close')) {
                fecharModal(e.target.closest('.modal')); // Fecha o modal pai do botão de fechar
            }
        });
    });

    btnBuscarColaborador.addEventListener('click', () => {
        if (buscaColaboradorSelect.value) {
            buscarDadosColaborador(buscaColaboradorSelect.value);
        } else {
            alert('Por favor, selecione um colaborador para ver o relatório.');
        }
    });
    btnPagarHoras.addEventListener('click', pagarHoras);
    btnDownloadPdf.addEventListener('click', gerarPDF);
});