...
class ProximoDaVezService {
  static const List<String> TODOS_COLABORADORES = ['Maviael', 'Raminho', 'Bruninho', 'Matheus', 'Isaac', 'Mikael'];
  static const List<String> COLABORADORES_ROTACAO = ['Maviael', 'Raminho', 'Bruninho', 'Matheus', 'Isaac'];
  ...
  Future<String> determinarProximo() async {
    try {
      ...
      // 3. Se não sobrar ninguém, retorna
      if (colaboradoresConsiderados.isEmpty) {
        print('No collaborators available after daily filter.');
  ...
  Future<List<Map<String, dynamic>>> _getHistorico() async {
    try {
      final response = await Supabase.instance.client.from('historico').select();
      print('Supabase Historico Raw Response: $response'); // Adicionado para depuração
  ...
  String _desempatar(List<String> empatados, List<Map<String, dynamic>> historico) {
    ...
    contagemSede.forEach((nome, count) {
      pontuacao[nome] = pontuacao[nome]! + (count / 2).floor();
    });
