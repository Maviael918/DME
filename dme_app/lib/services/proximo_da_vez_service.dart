...
import 'package:dme_app/screens/separacao_merendas_menu_screen.dart';
...
class ProximoDaVezService {
  ...
  Future<String> determinarProximo() async {
    try {
      ...
      // Corrige duplicidade: conta apenas registros únicos por colaborador por viagem
      // Para Sede: só troca após 2 saídas no mesmo dia
      // Para outros setores: troca após 1 saída
      for (var c in colaboradoresRotacao) {
        final sedeCount = contagemViagensHoje[c]!['sede']!;
        final outrosCount = contagemViagensHoje[c]!['outros']!;
        if (sedeCount == 1 && outrosCount == 0) {
          // Se saiu 1 vez para Sede e 0 para outros, continua sendo o da vez
          return c;
        }
      }
      ...
      colaboradoresConsiderados.removeWhere((c) {
        final sedeCount = contagemViagensHoje[c]!['sede']!;
        final outrosCount = contagemViagensHoje[c]!['outros']!;
        // Para Sede: remove se saiu 2 vezes
        // Para outros: remove se saiu 1 vez
        return sedeCount >= 2 || outrosCount >= 1;
      });
  ...
  String _desempatar(List<String> empatados, List<Map<String, dynamic>> historico) {
    ...
    // Se ainda houver empate, ordena alfabeticamente para desempate estável e retorna o primeiro
    menosIdasSede.sort();
